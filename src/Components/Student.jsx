            ...
            <ul>
                {
                    stdlist.map((item,index)=>{
                        return(
                            ...
                            <button className="bg-black  py-1 px-4 rounded-md mr-5  text-red-500" onClick={()=>{handleaddfav(item)}}>Added</button>
                            </li>:<li key={index} className="flex mt-5 pl-5 justify-between lg:mx-auto lg:w-6/12	 align-middle">
                            <p className="text-2xl">{index+1}.{item.stdname}</p>
                            <button className="bg-black text-white py-1 px-4 rounded-md mr-5" onClick={()=>{handleaddfav(item)}}>Add</button>
                            </li>
                        )
                    })
                }
